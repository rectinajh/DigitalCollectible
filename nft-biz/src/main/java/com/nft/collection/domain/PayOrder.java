...
public class PayOrder implements Serializable {
	...
	public static PayOrder buildWithResaleCollection(MemberResaleCollection memberResaleCollection, Member member,
			Integer payOrderDeadline) {
		PayOrder po = new PayOrder();
		po.setId(IdUtils.getId());
		po.setOrderNo(po.getId());
		po.setCreateTime(new Date());
		po.setOrderDeadline(DateUtil.offset(po.getCreateTime(), DateField.MINUTE, payOrderDeadline));
	...
	public static PayOrder buildWithLatestCollection(Collection collection, Member member, Integer payOrderDeadline) {
		PayOrder po = new PayOrder();
		po.setId(IdUtils.getId());
		po.setOrderNo(po.getId());
		po.setCreateTime(new Date());
