...
public class ExchangeCode implements Serializable {
	...
	public void used(String issuedCollectionId, String memberId) {
		this.setState(Constant.兑换码状态_已使用);
		this.setExchangeTime(new Date());
		this.setIssuedCollectionId(issuedCollectionId);
		this.setMemberId(memberId);
	}

	public static ExchangeCode build(String collectionId, Date createTime, Integer effectiveDays) {
		ExchangeCode po = new ExchangeCode();
		po.setId(IdUtils.getId());
		po.setCode(RandomUtil.randomString(8));
		po.setState(Constant.兑换码状态_未使用);
		po.setCreateTime(createTime);
		if (effectiveDays != null && effectiveDays > 0) {
			po.setEffectiveTime(DateUtil.offset(po.getCreateTime(), DateField.DAY_OF_YEAR, effectiveDays).toJdkDate());
		}
