...
@Setter
@Entity
@Table(name = "first_order_reward_record")
@DynamicInsert(true)
@DynamicUpdate(true)
public class FirstOrderRewardRecord implements Serializable {
	...
	@NotFound(action = NotFoundAction.IGNORE)
	@ManyToOne(fetch = FetchType.LAZY)
	@JoinColumn(name = "pay_order_id", updatable = false, insertable = false, foreignKey = @ForeignKey(value = ConstraintMode.NO_CONSTRAINT))
	...
	public static FirstOrderRewardRecord build(String memberId, String collectionId, String payOrderId) {
		FirstOrderRewardRecord po = new FirstOrderRewardRecord();
