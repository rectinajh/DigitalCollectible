		...
		Specification<MemberBalanceChangeLog> spec = new Specification<MemberBalanceChangeLog>() {
			...
			public Predicate toPredicate(Root<MemberBalanceChangeLog> root, CriteriaQuery<?> query,
				...
				if (param.getTimeStart() != null) {
					predicates.add(
							builder.greaterThanOrEqualTo(root.get("changeTime").as(Date.class), param.getTimeStart()));
				}
				if (param.getTimeEnd() != null) {
					predicates
							.add(builder.lessThanOrEqualTo(root.get("changeTime").as(Date.class), param.getTimeEnd()));
				}
