...
var appVM = new Vue({
	...
	data : function() {
		return {
			...
			settlementAccountDialogFlag : false,
			settlementAccounts : [],
			showPreSaleDialogFlag : false,
			preMinute : '',
		}
	},
	mounted : function() {
	...
	methods : {
		...
		findSettlementAccount : function() {
			var that = this;
			that.$http.get('/member/findSettlementAccount', {
				params : {
					memberId : that.selectedAccountId,
				}
			}).then(function(res) {
				that.settlementAccounts = res.body.data;
			});
		},

		showSettlementAccountDialog : function(memberId) {
			var that = this;
			that.selectedAccountId = memberId;
			that.settlementAccountDialogFlag = true;
			that.findSettlementAccount();
		},
		...
		addPreSaleQualify : function() {
			...
			if (that.preMinute === null || that.preMinute === '') {
				that.$q.notify({
					type : 'negative',
					message : '请输入优先时间(分钟)'
				});
			...
			that.$http.post('/preSale/addPreSaleQualify', {
				collectionId : that.collectionId,
				memberId : that.selectedAccountId,
				preMinute : that.preMinute
			}, {
				emulateJSON : true
			}).then(function(res) {
				that.$q.notify({
				...
				that.showPreSaleDialogFlag = false;
				that.refreshTable();
			});
		},

		showPreSaleDialog : function(id) {
			this.showPreSaleDialogFlag = true;
