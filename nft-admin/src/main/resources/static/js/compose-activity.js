...
var appVM = new Vue({
	...
	data : function() {
		return {
			...
			composeRecords : [],
			memberMobile : '',
			showAdjustComposeActivityTimeDialogFlag : false,
		}
	},
	mounted : function() {
		var that = this;
	...
	methods : {
		...
		adjustComposeActivityTime : function() {
			var that = this;
			...
			that.$http.post('/composeActivity/adjustComposeActivityTime', selectedRecord, {
				emulateJSON : true
			}).then(function(res) {
				that.$q.notify({
					progress : true,
				...
				that.showAdjustComposeActivityTimeDialogFlag = false;
				that.refreshTable();
			});
		},

		showAdjustComposeActivityTimeDialog : function(record) {
			var that = this;
			that.selectedRecord = record;
			that.showAdjustComposeActivityTimeDialogFlag = true;
		},
