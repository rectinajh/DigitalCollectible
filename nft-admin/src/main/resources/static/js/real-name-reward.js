...
var appVM = new Vue({
	...
	methods : {
		...
		updateSetting : function() {
			...
			that.$http.post('/realNameReward/updateRealNameRewardSetting', setting, {
				emulateJSON : true
			}).then(function(res) {
				that.$q.notify({
					progress : true,
		...
		showSettingDialog : function(id) {
			var that = this;
			that.$http.get('/realNameReward/getRealNameRewardSetting', {
				params : {}
			}).then(function(res) {
		...
		loadTableData : function(param) {
			...
			that.$http.get('/realNameReward/findRewardRecordByPage', {
				params : {
