	...
	<view class="page-content">
		<u-popup v-model="showPayModalFlag" mode="bottom" border-radius="14" :closeable="true">
			<view class="pay-modal">
				<view class="pay-modal-amount">
					<text>￥</text><text
						class="pay-modal-amount-value">{{moneyFormat(collectionDetail.resalePrice)}}</text>
				</view>
		...
		<view class="fixed-bottom">
			<view class="fixed-bottom-content">
				...
				<view>
					<view class="buy-btn" @click="resaleCollectionCreateOrder" v-show="collectionDetail && canBuy()">
						立即购买</view>
					<view class="cannot-buy-btn" v-show="collectionDetail && !canBuy()">商品锁定中，待支付...</view>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			canBuy() {
				...
				return lockPayMemberId == accountId;
			},
			...
			getCollectionDetail() {
				var that = this;
				this.$u.get('/collection/findResaleCollectionDetail', {
